...
// GenerateFormula generates a formula file to tap repo and `git push“ it
func GenerateFormula(formulaTemplate, tapOwner, tapRepo, artifactOwner, artifactRepo, gitTag string) error {
	...
	release, err := GetTagRelease(&httpClient, artifactOwner, artifactRepo, gitTag)
	if err != nil {
	...
	formulaFilePath := fmt.Sprintf("%s/Formula/%s.rb", tapRepo, artifactRepo)
