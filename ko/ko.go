...
func KoInstall() error {
	return util.RunCmdWithLog("go install github.com/google/ko@latest")
}

// Build & publish image with ko
func KoPublish(importPath string) error {
	imageTag, err := util.GenerateImageTag()
	if err != nil {
	...
	out, errMsg, err := util.RunLongRunningCmdWithLog(koBuildCmd)
	if err != nil {
